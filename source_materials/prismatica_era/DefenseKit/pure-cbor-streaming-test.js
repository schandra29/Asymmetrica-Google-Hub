...
function generateNetflixChunk(size, chunkIndex) {
  ...
  const dataSize = Math.floor(size * 0.7); // 70% actual data
  const data = [];

  // Create patterns that compress well (like real video)
  for (let i = 0; i < dataSize; i += 100) {
    const blockSize = Math.min(100, dataSize - i);
    // I-frame, P-frame, B-frame pattern
    const frameType = i % 300 < 100 ? 'I' : (i % 300 < 200 ? 'P' : 'B');
    data.push({ frame: frameType, data: new Array(blockSize).fill(i % 256) });
  }
...
export default runPureCBORStreamingTest;